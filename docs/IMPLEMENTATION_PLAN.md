...
**File to create:** `src/lib/actions/clickOutside.ts` (new `actions/` directory)
...
Both dropdowns benefit from filtering: plans lists can be large, and task lists
within a plan routinely exceed 30 items.
...
**Template change** — replace both `<Select …>` with `<SearchableSelect …>`.
All props (`id`, `options`, `bind:value`, `placeholder`, `onchange`) remain
identical; only the component name changes.

No changes to `onPlanChange`, `onTaskChange`, `$effect` blocks, or any other
logic.
...
prepended "All plans" / "All tasks" options as selectable entries (from
TASK-44); `SearchableSelect` handles these correctly because the filter matches
against `label`, and an empty query shows all options unchanged.

---

#### 12.1.5 Update `src/lib/components/ui/index.ts` (if barrel file exists)

If a barrel export file exists, add:
